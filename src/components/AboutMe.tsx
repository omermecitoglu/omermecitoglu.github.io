...
const AboutMe = () => (
  <section id="about" className="min-vh-100">
    <div className="container py-3">
      <h2>About Me</h2>
      <hr />
      <p>👋 Hi there! My name is {(process.env.PROFILE_NAME || "John Doe").split(" ")[0]}, a passionate and creative Frontend Developer with a keen eye for design and a love for crafting seamless user experiences. 🚀</p>
      <p>With {new Date().getFullYear() - (parseInt(process.env.CAREER_START_YEAR || "") || 1970)} years of hands-on experience in web development, I've honed my skills in building captivating and responsive websites that not only look stunning but also perform flawlessly across various devices and browsers. My expertise lies in HTML, CSS, JavaScript, and the latest frontend frameworks like React.</p>
      ...
      <p>📬 Feel free to reach out to me via LinkedIn messaging or email at <Link href={`mailto:${process.env.PROFILE_EMAIL || "john.doe@example.com"}`}>{process.env.PROFILE_EMAIL || "john.doe@example.com"}</Link>. Let's connect and build something incredible!</p>
    </div>
  </section>
);
